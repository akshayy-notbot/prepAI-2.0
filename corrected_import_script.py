#!/usr/bin/env python3
"""
Corrected import script that stores JSON fields as direct content instead of wrapped structures.
...
def main():
    ...
    print("🚀 Starting corrected playbook import...")
    ...
    try:
        ...
        for i, values_content in enumerate(matches):
            try:
                ...
                if len(values) >= 12:
                    ...
                    # Store JSON fields as direct content (not wrapped)
                    # The AI agents expect to access these directly as dicts/lists
                    evaluation_dimensions_json = evaluation_dimensions
                    seniority_criteria_json = seniority_criteria
                    good_vs_great_examples_json = good_vs_great_examples
                    ...
                    with engine.connect() as connection:
                        ...
                        connection.execute(insert_sql, {
                            ...
                            'evaluation_dimensions': evaluation_dimensions_json,
                            'seniority_criteria': seniority_criteria_json,
                            'archetype': archetype,
                            'interview_objective': interview_objective,
                            'good_vs_great_examples': good_vs_great_examples_json,
        ...
        with engine.connect() as connection:
            ...
            if sample:
                ...
                print(f"\n📋 Sample evaluation_dimensions content:")
                print(f"Type: {type(sample[4])}")
                if isinstance(sample[4], str):
                    print(f"Content: {sample[4][:200]}...")
                else:
                    print(f"Content: {sample[4]}")
